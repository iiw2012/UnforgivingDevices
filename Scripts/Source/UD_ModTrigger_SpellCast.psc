...
Bool Function SpellCast(UD_Modifier_Combo akModifier, UD_CustomDevice_RenderScript akDevice, Spell akSpell, String asDataStr, Form akForm1)
    ...
    If TriggerOnValueDelta(akDevice, akModifier.NameAlias, asDataStr, afValueDelta = loc_cost, afMinAccum = loc_min_value, afProbBase = loc_prob_base, afProbDelta = loc_prob_delta, afProbAccum = loc_prob_acc, abRepeat = loc_repeat, aiAccumParamIndex = 5)
        Return True
    Else
...
String Function GetParamsTableRows(UD_Modifier_Combo akModifier, UD_CustomDevice_RenderScript akDevice, String asDataStr, Form akForm1)
    ...
    Float loc_prob_acc      = GetParamFlt(akModifier, akDevice, asDataStr, 3, 0.0,    "Probability")
    Bool loc_repeat         = GetParamBln(akModifier, akDevice, asDataStr, 4, False)
    Float loc_acc           = GetParamFlt(akModifier, akDevice, asDataStr, 5, 0.0)
