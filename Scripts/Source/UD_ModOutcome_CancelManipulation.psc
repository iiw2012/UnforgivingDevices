;/  File: UD_ModOutcome_CancelManipulation
    Cancels manipulations on devices

    NameFull: Cancel Manipulation
    ...
    Parameters in DataStr (indices relative to DataStrOffset property):
        ...
        [+1]    String      (optional) Selection method (in general or for the devices in list akForm2)
                            ...
                            Default value: S (SELF)

        [+2]    String      (optional) Selection method for the devices in list akForm3

    Form arguments:
        Form2               Single device keyword to cancel manipulation or FormList with keywords.

        Form3               Single device keyword to cancel manipulation or FormList with keywords.

    Example:
        ,,,,,,,1,FIRST      One device will be selected. The first suitable device from the list in Form4 will be selected by its keyword, 
                            starting from the top one.
        ,,,,,,,5,F,R        Five devices will be selected. First the matching devices will be selected from the list in Form4, and 
                            then the remaining number will be selected randomly from the list in Form5.
/;
Scriptname UD_ModOutcome_CancelManipulation extends UD_ModOutcome
...
Function Outcome(UD_Modifier_Combo akModifier, UD_CustomDevice_RenderScript akDevice, String asDataStr, Form akForm2, Form akForm3)
    ...
    String loc_method_list2     = GetParamStr(akModifier, akDevice, asDataStr, 1, "S")
    ...
    If loc_devices.Length > 1
        PrintNotification(akDevice, "You heard the sound of locks clicking shut on one of your devices that you had adjusted earlier.", aiEffectId = 0)
    ElseIf loc_devices.Length > 0
        PrintNotification(akDevice, "You heard the sound of locks clicking shut on several of your devices that you had adjusted earlier.", aiEffectId = 0)
    EndIf
